...
class WSHandler(tornado.websocket.WebSocketHandler):
    ...
    def open(self):
        ...
        req = tornado.httpclient.HTTPRequest("wss://speech.platform.bing.com/speech/recognition/interactive/cognitiveservices/v1?format=simple&language="+LANGUAGE, headers={'X-ConnectionId': str(uuid4().hex), 'Authorization' : token})
        self.azure_future = tornado.websocket.websocket_connect(req, on_message_callback=self.on_return_message)
    @gen.coroutine
    def on_message(self, message):
        azure = yield self.azure_future
        if type(message) != str:
            headers = """path: audio\r\ncontent-type: audio/x-wav\r\nx-requestid: {}\r\nx-timestamp: {}\r\n\r\n""".format(self.req_id, datetime.datetime.utcnow().isoformat())
            ...
            headers = """path: speech.config\r\ncontent-type: application/json\r\nx-xequestid: {}\r\nx-timestamp: {}\r\n\r\n""".format(str(uuid4().hex), datetime.datetime.utcnow().isoformat())
            config = {
            "context": {
              "system": {
               "version": "0.1",
